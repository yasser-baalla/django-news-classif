    ...
    if request.method == "POST":
        ...
        for i in range(min(12, len(final_articles))):
            var = '{"id" :"' + str(final_articles[i].id_cat) + '", "source":"' + final_articles[i].source + '","title":"' + final_articles[i].title.replace('"', '') + '","date":"' + final_articles[i].date.strftime(
                '%d/%m/%Y - %H:%M:%S') + '","content":"' + final_articles[i].url + '","image":"' + final_articles[i].image + '","views" : "' + str(final_articles[i].views) + '","cat":' + re.sub("([a-z])_([a-z])", "\\1 \\2", str(categories_final[i]).replace("'", '"')) + '},'
            data = data + var
        if data == '{ "articleList" : [':
            data = '{ "dataError" : true }'
        else:
